...
public class CompaniaController {
	...
	@GetMapping("/companias/lista")

	public List<CompaniaReducidaDTO> obtenerListaCompanias() {
		ModelMapper modelMapper = new ModelMapper();
		return StreamSupport.stream(companiaService.obtenerCompanias().spliterator(), false)
				.map(c -> modelMapper.map(c, CompaniaReducidaDTO.class)).collect(Collectors.toList());
	}
